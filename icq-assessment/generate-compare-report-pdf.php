<?php
	...
	if (isset($_POST['cgid'])) $c_group_id = $_POST['cgid'];
	...
	if (isset($c_group_id))
	{
		$compare_group_added = $icq_dashboard->get_participant_added_by_group_id($c_group_id);
		$compare_group_name = $icq_dashboard->get_cohort_group_data($c_group_id)->group_name;
	}
	else exit;

	if (!isset($compare_group_name) || $compare_group_name == '' || $compare_group_name == NULL) exit;
	...
	for ($c = 1; $c <= 2; $c ++) {
		if ($c==1) {
			$_participant_added = $participant_added; // A single participant added who was compared.
			$idx_name = 'group1';
			
		} else if ($c==2) {
			$_participant_added = $compare_group_added; // Participants added in group.
			$idx_name = 'group2';
		}
	...
	$D_quadrant_value = $DISC_result['group1']['D_quadrant_value'];
	$I_quadrant_value = $DISC_result['group1']['I_quadrant_value'];
	$S_quadrant_value = $DISC_result['group1']['S_quadrant_value'];
	$C_quadrant_value = $DISC_result['group1']['C_quadrant_value'];
	$team1_sliding_scales_value = $DISC_result['group1']['sliding_scales_value'];
	$participant_counter_1 = $DISC_result['group1']['participant_counter'];

	$group_D_quadrant_value = $DISC_result['group2']['D_quadrant_value'];
	$group_I_quadrant_value = $DISC_result['group2']['I_quadrant_value'];
	$group_S_quadrant_value = $DISC_result['group2']['S_quadrant_value'];
	$group_C_quadrant_value = $DISC_result['group2']['C_quadrant_value'];
	$team2_sliding_scales_value = $DISC_result['group2']['sliding_scales_value'];
	$participant_counter = $DISC_result['group2']['participant_counter'];
	...
	for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
		...
		$compare_group_name = toAscii($compare_group_name);
		...
		if ($pageNo == 1) {
			...
			$pdf->MultiCell(75, 5.65, transformStringQuote($compare_group_name) , 0, 1, 'L');
			...
			$write_text = $group_name . ' (' . $compare_group_name . ')';
			...
			$pdf->SetXY(33, 64);
			$pdf->SetTextColor(41,40,57);
			$pdf->MultiCell(132, 5.65, $compare_group_name, 0, 1, 'L');
	...
	// Remove string space
	$compare_group_name = str_replace(' ', '', $compare_group_name);
	$group_name = str_replace(' ', '', $group_name);
	
	// Clean string
	$compare_group_name = $icq_report->cleanStr($compare_group_name);
	$group_name = $icq_report->cleanStr($group_name);
	
	$generation_result_path = 'pdf/Global-DISC-comparsion-'.$group_name.'-'.$compare_group_name.'.pdf';
