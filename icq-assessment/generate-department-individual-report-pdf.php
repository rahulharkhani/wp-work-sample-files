<?php
	...
	if (isset($_POST['uid'])) $individual_url_id = $_POST['uid'];
	...
	if ( in_array('licensee', $user_roles) ) {
		...
		$participant_added_1 = $icq_dashboard->get_participant_added_by_id($individual_url_id);
		$participant_added_2 = $icq_dashboard->get_participant_added_by_group_id($group_id, $Department);

		$single_participant_name = $participant_added_1[0]->participant_name;
		...
		$participant_added_1 = $icq_dashboard->get_participant_added_by_id($individual_url_id);
		$participant_added_2 = $icq_dashboard->get_participant_added_with_department($Department);
		$single_participant_name = $participant_added_1[0]->participant_name;
		$user = wp_get_current_user();
	...
	if (!isset($single_participant_name) || $single_participant_name == '' || $single_participant_name == NULL) exit;
	if (!isset($Department) || $Department == '' || $Department == NULL) exit;
	...
	$team1_sliding_scales_value = $DISC_result['Participant']['sliding_scales_value'];
	$participant_counter_1 = $DISC_result['Participant']['participant_counter'];

	$department_D_quadrant_value = $DISC_result['Department']['D_quadrant_value'];
	$department_I_quadrant_value = $DISC_result['Department']['I_quadrant_value'];
	$department_S_quadrant_value = $DISC_result['Department']['S_quadrant_value'];
	$department_C_quadrant_value = $DISC_result['Department']['C_quadrant_value'];
	$team2_sliding_scales_value = $DISC_result['Department']['sliding_scales_value'];
	$participant_counter = $DISC_result['Department']['participant_counter'];
	...
	for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
		...
		if ($pageNo == 1) {
			...
			$pdf->MultiCell(75, 5.65, transformStringQuote($Department) , 0, 1, 'L');
			...
			$write_text = $single_participant_name . ' (' . $Department . ')';
			...
			$pdf->Write(0, round($D_quadrant_value) . '(' . round($department_D_quadrant_value).')%');
			
			$pdf->SetXY(139.2, 182.5);
			$pdf->Write(0, round($I_quadrant_value) . '(' . round($department_I_quadrant_value).')%');
			
			$pdf->SetXY(139.2, 213.5);
			$pdf->Write(0, round($S_quadrant_value) . '(' . round($department_S_quadrant_value).')%');
			
			$pdf->SetXY(48.7, 213.5);
			$pdf->Write(0, round($C_quadrant_value) . '(' . round($department_C_quadrant_value).')%');
			...
			$pdf->SetXY(33, 64);
			$pdf->SetTextColor(41,40,57);
			$pdf->MultiCell(132, 5.65, $Department, 0, 1, 'L');
	...
	// Clean string
	$Department = $icq_report->cleanStr($Department);
	$single_participant_name = $icq_report->cleanStr($single_participant_name);
	
	$generation_result_path = 'pdf/Global-DISC-comparsion-'.$single_participant_name.'-'.$Department.'.pdf';
