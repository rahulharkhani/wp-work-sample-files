<?php
	...
	if (isset($group_id))
	{
		$compare_group_added = $icq_dashboard->get_participant_added_by_group_id($group_id);
		$compare_group_name = $icq_dashboard->get_cohort_group_data($group_id)->group_name;
	}
	else exit;

	if (!isset($POSTID) || $POSTID == '' || $POSTID == NULL) exit;
	...
	$group_D_quadrant_value = $DISC_result['group1']['D_quadrant_value'];
	$group_I_quadrant_value = $DISC_result['group1']['I_quadrant_value'];
	$group_S_quadrant_value = $DISC_result['group1']['S_quadrant_value'];
	$group_C_quadrant_value = $DISC_result['group1']['C_quadrant_value'];
	$team2_sliding_scales_value = $DISC_result['group1']['sliding_scales_value'];
	$participant_counter = $DISC_result['group1']['participant_counter'];
	...
	for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
		...
		if ($pageNo == 1) {
			...
			$write_text = $CountryName . ' (' . $group_name . ')';
	...
	$generation_result_path = 'pdf/Global-DISC-comparsion-'.$CountryName.'-'.$group_name.'.pdf';
